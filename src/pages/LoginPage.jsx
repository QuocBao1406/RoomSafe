...
function LoginPage() {
  ...
  const onLoginSubmit = async (data) => {
    try {
      ...
      // Lưu token vào localStorage riêng lẻ nếu cần thiết cho các config axios khác
      if (userData.token) {
        localStorage.setItem('token', userData.token);
      }

      // tao object luu user vao localStorage (bao gom token cho admin)
      const localUser = {
        id: userData.id,
      ...
      setTimeout(() => {
        if (localUser.role === 'ADMIN') {
          navigate('/admin');
        } else if ( localUser.role === 'LANDLORD') {
          navigate('/dashboard');
        } else {
          navigate('/');
        }
      ...
      <div className={`login-register-container ${isActive ? "active" : ""}`}>
        ...
        <div className="toggle-box">
          <div className="toggle-panel toggle-right">
            ...
            <p>
              Nhập thông tin cá nhân để sử dụng tất cả tính năng của RoomSafe
            </p>
            <button
