...
import { FaTrash, FaEdit, FaPlus, FaMapMarkerAlt, FaClock, FaVenusMars, FaUserFriends, FaCheckCircle, FaBriefcase, FaLayerGroup } from "react-icons/fa";
import roommateApi from "../services/roommateApi.js"; 
import { UserContext } from "../contexts/UserContext.jsx";
import "../css/RoommateManage.css"; 
// 1. IMPORT MODAL
import RoommateDetailModal from "../components/RoommateDetailModal.jsx";
      ...
      <div className="rm-container">
        ...
        {items.length === 0 ? (
            ...
            {items.map((item) => {
                ...
                return (
                // 5. THÊM SỰ KIỆN CLICK VÀO CARD
                <div key={item.post_id} className="rm-card" onClick={() => handleOpenModal(item)} style={{cursor: 'pointer'}}>
                        ...
                        <div className="rm-tags">
                            ...
                            {(details.age_range_min || details.age_range_max) && (
                                <div className="rm-tag age">
                                    <FaUserFriends/> {details.age_range_min || '..'} - {details.age_range_max || '..'} tuổi
                                </div>
                            )}
                            {details.career && (
                                <div className="rm-tag" style={{background:'#f3f4f6', color:'#4b5563'}}>
                                    <FaBriefcase/> {details.career === 'SINH_VIEN' ? 'Sinh viên' : 'Người đi làm'}
                                </div>
                            )}
                        </div>
                    </div>
                    ...
                    <div className="rm-footer">
                        <div 
                            ...
                            onClick={(e) => handleToggleStatus(e, item)} // Chú ý truyền e
                        ...
                        <div className="rm-actions">
                            <button className="rm-btn-icon" title="Sửa tin" onClick={(e) => { e.stopPropagation(); navigate(`/roommate/edit/${item.post_id}`) }}>
                                <FaEdit />
                            </button>
                            <button className="rm-btn-icon delete" title="Xóa tin" onClick={(e) => handleDelete(e, item.post_id)}>
                                <FaTrash />
                            </button>
                        </div>
                    </div>
